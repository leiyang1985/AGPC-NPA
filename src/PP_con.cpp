PQ_mat_struct PP_con_cal(PQ_mat_struct &P1,PQ_mat_struct &P2)
{
	PQ_mat_struct temp;
	temp.M=P2.M-P1.M;
	...
	for(int i=1;i<=P2.val[0];i++)
	{
		for(int j=1;j<=P1.val[0];j++)
		{
			...
			if(ser[size_P2-1]==P1.ser[j][1])
			{
				size-=2;
				ser[0]=size;
				ser[size_P2]+=P1.ser[j][2];
				memcpy(ser+size_P2+1,P1.ser[j]+3,sizeof(int)*(size_P1-2));
			}
			else
			{
